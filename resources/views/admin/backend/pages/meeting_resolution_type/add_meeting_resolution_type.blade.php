    ...
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body p-4">
                <h5 class="card-title mb-4">เพิ่มรายงานมติที่ประชุม</h5>
                <hr/>
                                ...
                                <div class="row g-3">
                                    <div class="form-group col-md-6">
                                        ...
                                        <select name="management_category_id" id="management_category_id" class="form-select">
                                            <option value="" selected disabled>-- เลือกเลขหมวดด้านการบริหาร --</option>
                                            @foreach ($managementCategories as $category)
                                                <option value="{{ $category->id }}" {{ old('management_category_id') == $category->id ? 'selected' : '' }}>{{ $category->category_code }} - {{ $category->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="text" class="form-control" name="meeting_no" id="meeting_no" placeholder="เช่น 1" value="{{ old('meeting_no') }}">
                                    </div>

                                    <div class="form-group col-md-4">
                                        <label for="meeting_year" class="form-label">ปี พ.ศ. <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" name="meeting_year" id="meeting_year" placeholder="เช่น 2567" value="{{ old('meeting_year') }}">
                                    </div>

                                    <div class="form-group col-md-4">
                                        <label for="meeting_date" class="form-label">วันที่ประชุม <span class="text-danger">*</span></label>
                                        <input type="date" class="form-control" name="meeting_date" id="meeting_date" value="{{ old('meeting_date') }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="row g-3">
                                    <div class="form-group col-md-12">
                                        ...
                                        <select name="name" id="name" class="form-select">
                                            <option value="" selected disabled>-- เลือกเรื่องพิจารณา --</option>
                                            @foreach ($managementKeywords as $keyword)
                                                <option value="{{ $keyword->keyword_title }}" {{ old('name') == $keyword->keyword_title ? 'selected' : '' }}>{{ $keyword->keyword_title }} ({{ $keyword->managementCategory->category_code }})</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-md-12">
                                        ...
                                        <input type="text" class="form-control" name="agenda_title" id="agenda_title" placeholder="ระบุระเบียบวาระที่เกี่ยวข้อง" value="{{ old('agenda_title') }}">
                                    </div>

                                     <div class="form-group col-md-12">
                                        <label for="resolution_text" class="form-label">มติที่ประชุม <span class="text-danger">*</span></label>
                                        <textarea name="resolution_text" id="resolution_text" class="form-control" rows="5">{{ old('resolution_text') }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                         ...
                         <div class="card shadow-sm border-0">
                             ...
                             <div class="card-body">
                                 <div class="row g-3">
                                    <div class="form-group col-md-6">
                                        ...
                                        <div class="mt-2">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="task_status" id="task_status_completed" value="completed" {{ old('task_status') == 'completed' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="task_status_completed">
                                                    ดำเนินการแล้ว
                                                </label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="task_status" id="task_status_in_progress" value="in_progress" {{ old('task_status') == 'in_progress' ? 'checked' : '' }}>
                                                <label class="form-check-label" for="task_status_in_progress">
                                                    อยู่ระหว่างดำเนินการ
                                                </label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="task_status" id="task_status_not_started" value="not_started" {{ old('task_status', 'not_started') == 'not_started' ? 'checked' : '' }}>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label for="document" class="form-label">เอกสารแนบ (ถ้ามี)</label>
                                        <input type="file" class="form-control" name="document" id="document" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.jpg,.jpeg,.png">
                                        <small class="text-muted">รองรับ: PDF, Word, Excel, PowerPoint, รูปภาพ (สูงสุด 10MB)</small>
                                    </div>
                                 </div>
                            </div>
                        </div>
        ...
        $(document).ready(function() {
            ...
            $('#resolutionTypeForm').validate({
                rules: {
                    ...
                    document: { // Validation for file upload
                ...
                messages: {
                    ...
                    name: { required: 'กรุณาเลือกเรื่องพิจารณา' },
